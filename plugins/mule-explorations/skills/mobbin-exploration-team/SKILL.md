...
**The survey is looking at the screenshots, not collecting app names.** Read measurable structure off the image and carry that into the design itself. On the page, the survey surfaces only as each concept's references line (`Link 1`, `Link 2` → the `mobbin_url`s that informed it). Save the structural facts and app names for the chat hand-off. Never copy markup or source assets from Mobbin — tokens and conventions still come from this skill.
